...
local function show_pathfinding_tile_nearest_entity(command)
...
local function recalculate_disjoint_tiles(data)
...
local function show_disjoint_tile_set(data)
    ...
    if ds then
        for k, v in pairs(ds.parent) do
            ...
            util.highlight_position(surface, pos, string_to_rgb(ds:find(k)), true)
        end
    end
end

---@param data CustomCommandData
local function test_two_tiles(data)
