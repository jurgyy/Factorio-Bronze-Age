...
function util.hslToRgb(h, s, l)
    ...
    local r = util.hue2rgb(p, q, h + 1/3)
    local g = util.hue2rgb(p, q, h)
    local b = util.hue2rgb(p, q, h - 1/3)

    return math.floor(r * 255), math.floor(g * 255), math.floor(b * 255)
end

function util.intToRainbowColor(i, cycle_length)
    local h = (i % cycle_length) / cycle_length
    local s = 1.0
    local l = 0.5
